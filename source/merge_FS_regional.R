analysisplan <- read.csv("input/dap_merge_FS_part1_region.csv", stringsAsFactors = F)
...
big_big_table <- results_hc_idp$results %>% lapply(function(x) x[["summary.statistic"]]) %>% do.call(rbind, .) %>%
  select(dependent.var, independent.var, dependent.var.value, independent.var.value, numbers, repeat.var.value)

variable_to_split_log_by <- "repeat.var.value"
each_group <- big_big_table[[variable_to_split_log_by]]
big_table_region <- big_big_table %>% split.data.frame(list(each_group))

write_regional_merge <- function(big_table) {
  ...
  demo <- response_hc_idp %>%
    filter(region == big_table$repeat.var.value[1]) %>%
    #makes category
  ...
  merge$repeat.var.value <- big_table$repeat.var.value[1]
  return(merge)
}

data_merge_region <- lapply(big_table_region, write_regional_merge) %>% do.call(bind_rows, .)


page_order_function <- function(merge){
  order_lsg <- c("wash_score", "health_score", "snfi_score", "edu_score", "prot_score",
                 "mcsi_score", "pev_score", "impact_score")
...
page_order <- page_order_function(data_merge_region)

data_merge_region <- data_merge_region[, c("repeat.var.value", page_order)]
data_merge_region <- data_merge_region[, names(data_merge_region)[!(names(data_merge_region) %in%  grep("\\.[1-9]$", names(data_merge_region), value = T))]] #removing the duplicates with the big_bar

data_merge_region[is.na(data_merge_region)] <- 0

data_merge_region %>% write.csv("output/large_table_part1_region.csv", row.names = F)
